...
```
Anonymises IP addresses in web server log files

Usage: log-anonymiser [OPTIONS] <INPUT> <OUTPUT>

Arguments:
  <INPUT>   Input log file
  <OUTPUT>  Output log file (must not exist)
...
Options:
      ...
      --dns.timeoutmillis <DNS_TIMEOUTMILLIS>
          DNS lookup timeout in milliseconds [default: 30000]
      --dns.cachemaxsize <DNS_CACHEMAXSIZE>
          Maximum number of DNS PTR lookup results to keep in the LRU cache [default: 100000]
      --threads <THREADS>
          Number of concurrent threads for parallel DNS lookups [default: 32]
      --mask.ipv4 <MASK_IPV4>
          How many bits in IPv4 addresses to mask/anonymise [default: 8]
      --mask.ipv6 <MASK_IPV6>
          How many bits in IPv6 addresses to mask/anonymise [default: 80]
      --compress.input <COMPRESS_INPUT>
          Is the input file gzip compressed true/false (default: autodetect from extension) [possible values: true, false]
      --compress.output <COMPRESS_OUTPUT>
          Should the output file be gzip compressed true/false (default: same as input) [possible values: true, false]
      --trace
          Log a trace line to stdout for every processed log row (line number, addresses masked, running total, and processing time in microseconds)
  -h, --help
          Print help
```
...
Releases are available on github: https://github.com/spatineo/log-anonymiser-2/releases/
